...
const COLORS: Array<[RegExp, string]> = [
  ...
  [/\b(gold|yellow|sun)\b/, "#f0d24a"],
  [/\b(lime|poison|venom|green)\b/, "#7db86a"],
  [/\b(teal|jade)\b/, "#4aa88a"],
  [/\b(ice|frost|cyan|aqua)\b/, "#9ad8ea"],
  [/\b(blue|azure|cobalt)\b/, "#6a8ec8"],
  [/\b(violet|purple|arcane)\b/, "#9a7ab8"],
  [/\b(pink|rose)\b/, "#d48aa8"],
  [/\b(white|holy|light)\b/, "#ecece8"],
  [/\b(shadow|void|black)\b/, "#3a3c3a"],
];
...
function remix(base: CraftedSpell): CraftedSpell {
  ...
  if (twist > 0.78) extra = EXTRAS[Math.floor(Math.random() * EXTRAS.length)]!;
  if (twist > 0.88) shape = SHAPES[Math.floor(Math.random() * SHAPES.length)]!;
